...
namespace raylibcpp {
	...
	public:
		...
		inline Window& SetIcon(raylib::Image image) {
		...
		inline raylib::Vector2 GetWindowPosition() {
			return raylib::GetWindowPosition();
		}

		std::string GetMonitorName(int monitor) {
			return std::string(raylib::GetMonitorName(monitor));
		}

		std::string GetClipboardText() {
			return std::string(raylib::GetClipboardText());
		}
